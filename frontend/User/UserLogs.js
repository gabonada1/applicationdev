...
export default function UserLogs() {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((r) => {
      const utensil = (r.utensil?.name || "").toLowerCase();
      const status = String(r.status || "").toLowerCase();
      return utensil.includes(q) || status.includes(q);
    });
  }, [records, query]);
      ...
      <View style={styles.card}>
        <View style={styles.rowTop}>
          <Text style={styles.name} numberOfLines={1}>
            {item.utensil?.name || "Unknown Utensil"}
          </Text>

          <View style={[styles.pill, isReturned ? styles.pillReturned : styles.pillBorrowed]}>
            <Text style={[styles.pillText, isReturned ? styles.pillReturnedText : styles.pillBorrowedText]}>
